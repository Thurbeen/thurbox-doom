...
return {
	...
	on_key = function(key)
		...
		-- agent. Swallow it instead. The reserved chords (`ctrl+q`, `f10`, `tab`,
		-- `ctrl+h`/`ctrl+l`, `f12`) never reach a plugin, so this cannot trap
		-- anyone.
		--
		-- Our own declared chord is let through, since the kernel resolves a
		-- declared chord to an action and this handler must not race that.
		if key and key.key and key.key == chord_for(LAUNCH) then
			return false
		end
		return true
	end,
